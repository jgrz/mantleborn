    ...
    async checkJobStatus(job) {
        ...
        try {
            ...
            if (job.status === 'completed' && oldStatus !== 'completed') {
                ...
                // For map_object, bitforge, and variants - use job result directly (no GET endpoint)
                if (job.type === 'map_object' || job.type === 'map_object_variant' || job.type === 'bitforge_variant') {
